...
public class MailboxDemo {
    ...
    public static void setComMailbox(String mail) throws Exception {
        String arr[] ={mail};//接收邮箱
        String subject = "绑定邮箱";//邮件主题
        String name = "java";//发件人昵称展示
        ...
        content.append("<body><div><h2>绑定邮箱</h2>亲爱的用户:您好!感谢您使用企业邮箱服务。您正在进行邮箱验证，请在验证码输入框中输入此次验证码（5分钟内有效)：<a href='javascript:;' >").append(nuberm).append("</a>以完成验证。如非本人操作，请忽略此邮件,由此给您带来的不便请谅解!</div>");
        content.append("<div><span style ='float: right;'>Java团队</span></div>");
